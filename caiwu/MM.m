...
% -------------------------------------------------------------------------
    ...
    hg(:,23) = backBuyRate;
    a = str2num(char(hg(:,15)));
    b = str2num(char(hg(:,23)));

    hg(:,24) = cellstr(num2str(a+b));
    hg(1:nRqhgR,25) = rqhg(:,1);
    hg(nRqghR+1:end,25) = rqgh(:,1);
    
    hg(1:nRqhgR,36) = rqhg(:,18);
    hg(nRqghR+1:end,36) = rqgh(:,18);
    hg(:,41) = cellstr('101');
    ...
    xlswrite(['hg',regexprep(fileName,'txt',''),'xls'],hg,1,'A2');
